app.controller('clashPlayerBase', ['$scope', 'getPlayers', '$route', '$timeout', function ($scope, getPlayers, $route, $timeout) {
  ...
  // PLAYER ROLE
  if ($scope.baseData.role === 'leader') { $scope.playerRole = 'Lider'; }
  else if ($scope.baseData.role === 'coLeader') { $scope.playerRole = 'Colider'; }
  else if ($scope.baseData.role === 'admin') { $scope.playerRole = 'Veterano'; }
  else if ($scope.baseData.role === 'member') { $scope.playerRole = 'Miembro'; }
	...
	else if($scope.townHall === 9) { $scope.townH = 'images/builderbase/BuilderHall9.png'; }
    else { $scope.townH = 'images/builderbase/BuilderHall1.png'; };

    // TROOPS ARRAY
    $scope.compareTroops = [
      { name: 'Raged Barbarian', village: 'builderBase', order: 1,'level': 0, 'maxLevel': 0, baseLevel: 1, endLevel: 18 },
      { name: 'Sneaky Archer', village: 'builderBase', order: 2,'level': 0, 'maxLevel': 0, baseLevel: 2, endLevel: 18 },
      { name: 'Boxer Giant', village: 'builderBase', order: 3,'level': 0, 'maxLevel': 0, baseLevel: 3, endLevel: 18 },
      { name: 'Beta Minion', village: 'builderBase', order: 4,'level': 0, 'maxLevel': 0, baseLevel: 3, endLevel: 18 },
      { name: 'Bomber', village: 'builderBase', order: 5,'level': 0, 'maxLevel': 0, baseLevel: 4, endLevel: 18 },
      { name: 'Baby Dragon', village: 'builderBase', order: 6,'level': 0, 'maxLevel': 0, baseLevel: 4, endLevel: 18 },
      { name: 'Cannon Cart', village: 'builderBase', order: 7,'level': 0, 'maxLevel': 0, baseLevel: 5, endLevel: 18 },
      { name: 'Night Witch', village: 'builderBase', order: 8,'level': 0, 'maxLevel': 0, baseLevel: 6, endLevel: 18 },
      { name: 'Drop Ship', village: 'builderBase', order: 9,'level': 0, 'maxLevel': 0, baseLevel: 7, endLevel: 18 },
      { name: 'Super P.E.K.K.A', village: 'builderBase', order: 10,'level': 0, 'maxLevel': 0, baseLevel: 8, endLevel: 18 },
	  { name: 'Hog Glider', village: 'builderBase', order: 11,'level': 0, 'maxLevel': 0, baseLevel: 9, endLevel: 18 }
    ];

    // HEROES ARRAY
    $scope.compareHeroes = [
      { name: 'Battle Machine', level: 0, maxLevel: 0, village: 'builderBase', endLevel: 30 }
    ];
